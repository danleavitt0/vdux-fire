  ...
  function addListener(_ref3) {
    ...
    if (type === 'once') {
      return dbref.once('value').then(function (snap) {
        ...
        return join ? db.ref(join.ref).child(value[join.child]).child(join.childParam).once('value').then(function (snap) {
          return snap.val();
        }).then(function (populateVal) {
          return dispatch((0, _actions.invalidate)({ ref: ref, name: name, value: (0, _extends4.default)({}, value, (0, _defineProperty3.default)({}, join.child, populateVal)) }));
...
function orderData(snap) {
  ...
  snap.forEach(function (child) {
    var val = child.val();
    ordered.push((0, _extends4.default)({}, val, { key: child.key }));
  });
  return ordered;
}
