...
function mw({ dispatch, getState, actions }) {
  ...
  function refBuilder(q) {
    ...
    const qp = pageSize
      ? queryParams.concat(`limitToFirst=${startAt ? pageSize + 1 : pageSize}`)
      : queryParams;
    ...
    if (orderBy.length > 1)
      throw new Error(
        "vdux-fire error: only one order by method is allowed per query"
      );
  ...
  function getLastItem(payload) {
    ...
    const newPayload = {
      ...builtQuery,
      pageSize: null,
      queryParams: (builtQuery.queryParams || []).concat("limitToLast=1")
    };
  ...
  function addListener(payload) {
    ...
    const bind =
      (queryParams && bindAs === "object") || !queryParams ? "object" : "array";
    if (type === "once") {
      return dbref
        .once("value")
    ...
    if (type === "limitData") {
      ...
      dbref.on("child_removed", snap =>
        dispatch(actions.removeKey({ name, key: snap.key }))
      );
    ...
    if (pageSize) {
      ...
      dbref.on("child_changed", (snap, prevSib) =>
        dispatchArrayMerge(snap, prevSib, true)
      );
      return;
    }
    dbref.on(
    ...
    function maybeJoin(value, arrJoin) {
      return join
        ? Promise.all(
            mapValues(j => joinResults(value, j, "on"), arrJoin)
          ).then(
            vals =>
              Array.isArray(join)
                ? vals.reduce(
    ...
    function dispatchResults(value) {
      dispatch(
        invalidate({ ref: url, name, value, mergeValues, page, childKey })
      );
      if (pageSize) {
...
function buildChildRef(value, ref, join) {
  ...
  if (typeof join.childRef === "function") {
    ...
    return typeof res === "object" && !res.database && !Array.isArray(res)
      ? Promise.props(
          map((val, key) => val.then(v => v).catch(() => {}), promisify(res))
        )
      : Promise.resolve(res);
...
function toPromise(ref, listener) {
  if (Array.isArray(ref)) {
    return mapValues(
      r => getPromise(r).then(s => ({ ...s.val(), key: s.key })),
      ref
    );
  } else if (typeof ref === "object" && !ref.database) {
    return map(
      (val, key) =>
        val ? getPromise(val).then(s => ({ ...s.val(), key: s.key })) : val,
      ref
    );
  } else {
    return getPromise(ref).then(
      s => (Array.isArray(s.val()) ? s.val() : { ...s.val(), key: s.key })
    );
  }
